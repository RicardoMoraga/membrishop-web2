...
# Idempotente: si ya corriste el script antes, la etiqueta o la rama pueden
# existir. Volver a crearlas aborta git; se detecta y se reutiliza el respaldo.
$tagExiste  = -not [string]::IsNullOrWhiteSpace((Leer-Git @('tag','-l',$tagRespaldo)))
$ramaExiste = -not [string]::IsNullOrWhiteSpace((Leer-Git @('branch','--list',$ramaRespaldo)))

if ($tagExiste) {
  Aviso "La etiqueta $tagRespaldo ya existe: se reutiliza, no se recrea."
} else {
...
if ($ramaExiste) {
  Aviso "La rama $ramaRespaldo ya existe: se reutiliza, no se recrea."
} else {
...
if (-not $SaltarBuild) {
  ...
  if (-not (Test-Path (Join-Path $Repo 'node_modules'))) {
    ...
    if (-not $Simular) {
      # npm.cmd y no npm: en Windows npm instala tres lanzadores (npm, npm.cmd,
      # npm.ps1) y PowerShell prefiere el .ps1, que la politica de ejecucion
      # bloquea con "no se puede cargar el archivo npm.ps1". Llamar al .cmd
      # esquiva el problema sin cambiar ninguna politica del sistema.
      & npm.cmd ci
...
# Si el arbol ya esta limpio, la ronda quedo commiteada en una corrida previa
# (o a mano) y aqui solo falta el push. No se crea un commit vacio.
$pendientes = Leer-Git @('status','--porcelain')
if ([string]::IsNullOrWhiteSpace($pendientes)) {
  Aviso "No hay cambios pendientes: la ronda ya esta commiteada."
  $adelanto = Leer-Git @('rev-list','--count','origin/main..HEAD')
  Bien "Commits locales sin subir: $adelanto"
  if ($adelanto -eq '0') { Bien "Nada que subir. Todo esta en origin/main."; exit 0 }
  Aviso "Se salta el commit y se pasa directo al push."
  $saltarCommit = $true
} else {
  $saltarCommit = $false
}
